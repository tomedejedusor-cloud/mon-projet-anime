...
document.addEventListener("DOMContentLoaded", function() {
    ...
    loaderTimeline.add({
        ...
        opacity: [0, 1],
        duration: 700,
        offset: '-=400' // En même temps que les autres boutons
    })
    ...
    loaderTimeline.finished.then(() => {
        ...
        let transitionTimeout; // Référence pour le timeout
        ...
        function startFinalTransition(instant = false) { 
            ...
            // Annule le timeout auto (si on a cliqué avant les 8s)
            ...
            if (instant) {
                ...
                // Comportement normal (avec glitch de 2s)
                ...
                glitchTimeline.finished.then(() => {
                    // pour que scene3D.js puisse le contrôler sans conflit.
                    console.log("Glitch terminé. Arrêt du contrôle Anime.js sur .spinner-wrapper.");
        ...
        // 3. Ajouter l'écouteur de clic (appelle avec 'true')
        const nextButton = document.querySelector('.btn-next-loader');
        if (nextButton) {
            nextButton.addEventListener('click', (e) => {
                e.preventDefault(); 
                startFinalTransition(true); // <-- Lance la transition instantanée
            });
        }
